...
export default function Cart({
  ...
  function normalizeMember(base, dist) {
    ...
    // ★ 這裡不自行計算點數，優先使用 Home 合併後帶入的 point / cashbackPoint
    const point =
      Number(base?.point ?? base?.cashbackPoint ?? base?.rewardPoints ?? 0);
    ...
    return {
      ...
      // 顯示用欄位
      memberType: base?.memberType ?? "",      // 中文（Home 會刷新）
      levelName: base?.levelName ?? "無",      // 等級字串（Home 會刷新）
      levelCode: base?.levelName ?? "無",      // 相容舊顯示
      point,                                   // 點數（Home 會刷新）
      cashbackPoint: point,
      rewardPoints: point,
  ...
  const actuallyRestore = async (order) => {
    // 點數安全檢查
    if ((Number(currentMember?.point ?? currentMember?.cashbackPoint) || 0) < usedPoints) {
      await Swal.fire({
        title: "點數不足",
        text: "會員點數不足以折抵",
        icon: "warning",
  ...
  return (
    <div className="cart py-3">
      <div className="w-100">
        ...
        >
          <div style={{ flex: 1 }}>
            <div className="d-flex align-items-center mb-1" style={{ fontWeight: 600 }}>
            ...
            {currentMember && (
              <div className="d-flex align-items-center text-muted small">
                <span className="me-3 d-inline-flex align-items-center">
                  ...
                  {currentMember?.memberType || "一般"}
                </span>
                <span className="me-3 d-inline-flex align-items-center">
                  <FaMedal className="me-1" />
                  {currentMember?.levelName ?? "無"} {/* 等級字串 */}
                </span>
                <span className="d-inline-flex align-items-center">
                  <FaTicketAlt className="me-1" />
                  點數：{Number(currentMember?.point ?? currentMember?.cashbackPoint ?? 0)}
                </span>
              </div>
            )}
          </div>

          <button className="change-button ms-3" onClick={() => setShowModal(true)}>
          ...
          <div className="d-flex align-items-center">
            <input
              ...
              onChange={(e) => {
                ...
                const maxByCashback = Number(currentMember?.point ?? currentMember?.cashbackPoint) || 0;
