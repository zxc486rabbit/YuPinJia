...
import ReceiveRecordModal from "./ReceiveRecordModal"; // 請確認此路徑正確
...
export default function Receive() {
  ...
  const handleDelete = (product) => {
    const confirmDelete = window.confirm(`確定要刪除「${product}」這筆領貨資料嗎？`);
    if (confirmDelete) {
      setReceiveList((prev) => prev.filter((item) => item.product !== product));
    }
  };
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      const parsed = jsonData
        .filter((row) => row["商品名稱"] && row["領貨日期"] && row["數量"])
        .map((row) => ({
          product: row["商品名稱"],
          date: row["領貨日期"],
      ...
      setReceiveList((prev) => [...prev, ...parsed]);
    };
        ...
        <div className="col-4 d-flex flex-column" style={{ background: "#fff", height: "89vh" }}>
          <div className="flex-grow-1">
            <div className="d-flex justify-content-between align-items-center my-3 mx-4">
              <h4 className="fw-bold m-0">領貨明細</h4>
              <div>
                <input
                  type="file"
                  accept=".xlsx,.xls"
            ...
            <table className="table mb-2" style={{ fontSize: "1.2rem" }}>
              <thead className="table-light" style={{ borderTop: "1px solid #c5c6c7" }}>
                <tr>
                  ...
                  <th className="text-center">領貨日期</th>
            ...
            <button
              ...
              onClick={() => {
                const confirmClear = window.confirm("確定要清空所有領貨明細嗎？");
                if (confirmClear) setReceiveList([]);
              }}
            >
              清空
            </button>
          ...
          <div
            ...
            style={{ fontSize: "1.3rem", fontWeight: "bold", lineHeight: "1.8", borderTop: "2px solid #E2E2E2" }}
          >
            <span>商品總數 : {receiveList.reduce((sum, item) => sum + item.quantity, 0)} 件</span>
            <button className="cargo-button">領貨</button>
          </div>
        </div>
        ...
        <div className="col-8">
          ...
          <div style={{ height: "76vh", overflow: "auto" }}>
            <table
              ...
              style={{ fontSize: "1.3rem", border: "1px solid #D7D7D7", width: "90%" }}
            >
              <thead
